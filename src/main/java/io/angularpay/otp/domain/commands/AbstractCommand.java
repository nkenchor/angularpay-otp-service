...
import io.angularpay.otp.exceptions.ValidationException;
import io.angularpay.otp.models.AccessControl;
...
import static io.angularpay.otp.helpers.Helper.*;
...
public abstract class AbstractCommand<T extends AccessControl, R> {
    ...
    public R execute(T request) {
        try {
            ...
            if (StringUtils.hasText(request.getAuthenticatedUser().getUserReference())) {
                boolean isResourceOwner = false;
                String resourceOwner = this.getResourceOwner(request);
                if (StringUtils.hasText(resourceOwner)) {
